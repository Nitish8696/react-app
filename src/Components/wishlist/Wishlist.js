...
const Wishlist = () => {
    ...
    return <>
        ...
        <div className='wishlist max-w-screen-2xl mx-auto grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-10 px-4 py-4'>
            {wishList?.map((item) => {
                return <div className='wish-pro-details border-[1px] border-gray-400 bg-white h-auto py-6 z-30
                 shadow-none hover:shadow-md hover:border-gray-200 duration-200 relative flex justify-center items-center flex-col gap-4' key={item.id}>
                    <CiCircleRemove className='wish-remove absolute top-2 right-3 cursor-pointer' onClick={() => handleClick(item.id)}/>
